...
(function () {
    ...
    function updateDisplay() {
        displayContainer.innerHTML = `
            <span style="color: aquamarine;">Speed: ${DEFAULT_VIDEO_SPEED.toFixed(2)}</span><br>
            <span style="color: lightcoral;">Saturation: ${DEFAULT_VIDEO_SATURATION.toFixed(2)}</span>
        `;
    ...
    function handlePressedKey(event) {
        ...
        switch (event.key) {
            ...
            case "]":
                ...
                break;
            case "{":
                DEFAULT_VIDEO_SATURATION = Math.max(MIN_SATURATION, DEFAULT_VIDEO_SATURATION - SATURATION_STEP);
                break;
            case "}":
                DEFAULT_VIDEO_SATURATION = Math.min(MAX_SATURATION, DEFAULT_VIDEO_SATURATION + SATURATION_STEP);
                break;
        ...
        if (newRate !== video.playbackRate) {
            ...
            DEFAULT_VIDEO_SPEED = newRate;
        }

        video.style.filter = `saturate(${DEFAULT_VIDEO_SATURATION})`;
        updateDisplay();
    }
    ...
    function updateDynamicAcceleration(video) {
        if (dynamicAcceleration.enable) {
            ...
            DEFAULT_VIDEO_SPEED = video.playbackRate;
